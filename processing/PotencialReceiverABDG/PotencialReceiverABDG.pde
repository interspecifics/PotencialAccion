...
void setup() {
  ...
  mCp5.addToggle("writeCSV")
    .setPosition(565, 110).setSize(80, 20)
      .setColorBackground(color( 255, 255, 255 )).setColorForeground(color(170, 170, 170)).setColorActive(color(170, 170, 170))
...
void oscEvent(OscMessage theOscMessage) {
  if (theOscMessage.addrPattern().startsWith("/emokit/")) {
    ...
    String addPat[] = theOscMessage.addrPattern().split("/");
    String sensorName = addPat[2];
    mSensors.get(sensorName).setFrequency((short)theOscMessage.get(0).floatValue(),
    (short)theOscMessage.get(1).floatValue(),
    (short)theOscMessage.get(2).floatValue(),
